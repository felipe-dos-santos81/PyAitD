...
def test_forced_found_choice_cannot_select_take():
    ...
    assert reduce_found(state, Action.LEFT, forced_refuse=True) is None
    assert state.choice is FoundResult.LEAVE
    assert reduce_found(state, Action.ACTION, forced_refuse=True) is FoundResult.LEAVE


def test_inventory_two_stage_selection_is_bounded():
    state = InventoryPresenter()
    reduce_inventory(state, Action.DOWN, object_ids=(4, 8), action_ids=(23, 25))
    assert state.object_cursor == 1
    assert reduce_inventory(state, Action.ACTION, object_ids=(4, 8), action_ids=(23, 25)) is None
    assert state.choosing_action is True
    reduce_inventory(state, Action.DOWN, object_ids=(4, 8), action_ids=(23, 25))
    result = reduce_inventory(state, Action.ACTION, object_ids=(4, 8), action_ids=(23, 25))
...
def test_character_selection_maps_left_to_emily_and_right_to_carnby():
    ...
    assert reduce_character_select(state, Action.ACTION) is None
    assert state.phase is CharacterPhase.STORY
    assert reduce_character_select(state, Action.INVENTORY_CONFIRM) == CharacterSelectResult(hero=1)
    state = CharacterSelectPresenter(choice=1)
    reduce_character_select(state, Action.ACTION)
    assert reduce_character_select(state, Action.ACTION) == CharacterSelectResult(hero=0)


def test_character_cancel_backs_out_then_quits():
    state = CharacterSelectPresenter(phase=CharacterPhase.STORY)
    assert reduce_character_select(state, Action.CANCEL) is None
    assert state.phase is CharacterPhase.PORTRAITS
    assert reduce_character_select(state, Action.CANCEL) == CharacterSelectResult(quit=True)


def test_system_main_wraps_and_opens_configuration():
    state = SystemMenuPresenter()
    reduce_system_menu(state, Action.UP, default_settings())
    assert state.cursor == 5
    reduce_system_menu(state, Action.DOWN, default_settings())
    assert state.cursor == 0
    state.cursor = 4
    assert reduce_system_menu(state, Action.INVENTORY_CONFIRM, default_settings()) is None
...
def test_configuration_toggles_capture_steals_and_escape_cancels():
    ...
    outcome = reduce_system_menu(state, Action.ACTION, default_settings())
    assert outcome.settings.sticky_action is True
    state.cursor = 1 + REMAPPABLE_CONTROLS.index(Control.ACTION)
    assert reduce_system_menu(state, Action.ACTION, outcome.settings) is None
...
def test_system_main_accept_rows_and_cancel():
    ...
    assert reduce_system_menu(state, Action.ACTION, default_settings()) == SystemMenuResult(
        close=True, save=True)
    state = SystemMenuPresenter(cursor=5)
    assert reduce_system_menu(state, Action.ACTION, default_settings()) == SystemMenuResult(
        quit=True, save=True)
    state = SystemMenuPresenter(cursor=1)
    assert reduce_system_menu(state, Action.CANCEL, default_settings()) == SystemMenuResult(
        close=True, save=True)


def test_main_save_load_and_quick_save_rows():
    state = SystemMenuPresenter(cursor=1)
    assert reduce_system_menu(state, Action.ACTION, default_settings()) is None
...
def test_load_page_enabled_slots_load_and_back():
    ...
    assert reduce_system_menu(
        state, Action.ACTION, default_settings(), slots) == SystemMenuResult(load_slot="manual")
    state = SystemMenuPresenter(page=SystemMenuPage.LOAD, cursor=1)
    assert reduce_system_menu(
        state, Action.ACTION, default_settings(), slots) == SystemMenuResult(load_slot="quick")
    state = SystemMenuPresenter(page=SystemMenuPage.LOAD, cursor=2)
    assert reduce_system_menu(state, Action.ACTION, default_settings(), slots) is None
    assert (state.page, state.cursor) == (SystemMenuPage.MAIN, 0)
...
def test_save_and_load_cancel_returns_to_main():
    for page in (SystemMenuPage.SAVE, SystemMenuPage.LOAD):
        ...
        assert reduce_system_menu(state, Action.CANCEL, default_settings()) is None
...
def test_graphics_and_realism_rows_cycle_render_options():
    ...
    # The len() halves of these were dropped when GRAPHICS_ROWS/REALISM_ROWS
    # became len() of their own cycle tuples. The literal counts stay: they
    # are what catches a row added or lost by accident.
    assert GRAPHICS_ROWS == 5
    ...
    assert reduce_system_menu(state, Action.ACTION, settings).settings.render == RenderOptions(scale=6)
    state.cursor = 1
    assert reduce_system_menu(state, Action.ACTION, settings).settings.render == RenderOptions(shading="flat")
    state.cursor = 2
    assert reduce_system_menu(state, Action.ACTION, settings).settings.render == RenderOptions(background_filter="xbr")
    state.cursor = 3
    assert reduce_system_menu(state, Action.ACTION, settings).settings.render == RenderOptions(msaa=8)
    state.cursor = 4
    assert reduce_system_menu(state, Action.ACTION, settings).settings.render == RenderOptions(smoothing=3)
    ...
    assert reduce_system_menu(state, Action.ACTION, settings).settings.render == RenderOptions(lighting="fixed")
    state.cursor = 1
    # shadows cycles hard -> soft -> room; the default is "soft", so one
    # ACCEPT from the default moves it to "room", not "hard".
    assert reduce_system_menu(state, Action.ACTION, settings).settings.render == RenderOptions(shadows="room")
    state.cursor = 2
    assert reduce_system_menu(state, Action.ACTION, settings).settings.render == RenderOptions(realism="classic")
    state.cursor = 3
    ...
    assert reduce_system_menu(state, Action.ACTION, settings).settings.render == RenderOptions(integration=3)
...
def test_the_realism_page_gained_the_occlusion_row():
    from PyAitD.app.ui import REALISM_CYCLES
...
def test_graphics_back_and_cancel_return_to_the_graphics_row_saving():
    ...
    assert reduce_system_menu(state, Action.ACTION, default_settings()) == SystemMenuResult(save=True)
    ...
    assert reduce_system_menu(state, Action.CANCEL, default_settings()) == SystemMenuResult(save=True)
...
def test_config_navigates_to_both_pages_and_back():
    ...
    settings = default_settings()
    assert reduce_system_menu(state, Action.ACTION, settings) is None
    assert state.page is SystemMenuPage.GRAPHICS and state.cursor == 0
    result = reduce_system_menu(state, Action.CANCEL, settings)
...
def test_realism_page_cycles_motion_and_backs_out():
    ...
    result = reduce_system_menu(state, Action.ACTION, settings)
...
def test_picker_ignores_keyboard_menu_commands_while_capturing():
    state = SystemMenuPresenter(page=SystemMenuPage.KEY_PICK, capture="ACTION", cursor=5)
    assert reduce_system_menu(state, Action.DOWN, default_settings()) is None
...
def test_pickable_keys_round_trip_through_pygame_and_fit_the_frame():
    ...
    from PyAitD.app.ui import PICKABLE_KEYS, SystemMenuLayout
    pygame.init()
    try:
