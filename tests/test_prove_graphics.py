...
def test_output_paths_cover_every_combination_plus_the_twins():
    ...
    names = {(name, mode, realism, level, shadows, integration, motion_blend)
             for name, mode, realism, level, shadows, integration, motion_blend, _ in paths}
    expected = {(n, m, r, default.smoothing, default.shadows, default.integration, False)
                for n in FIXTURES for m in SHADING_MODES for r in REALISM_MODES}
    expected |= {(n, "smooth", "enhanced", 0, default.shadows, default.integration, False) for n in FIXTURES}
    expected |= {(n, "smooth", "enhanced", default.smoothing, "hard", default.integration, False)
                 for n in FIXTURES}
    expected |= {(n, "smooth", "enhanced", default.smoothing, default.shadows, 0, False)
                 for n in FIXTURES}
    expected |= {(n, "smooth", "enhanced", default.smoothing, default.shadows, 3, False)
                 for n in FIXTURES}
    expected |= {(n, "smooth", "enhanced", default.smoothing, default.shadows, default.integration,
                  default.motion == "smooth")
                 for n in FIXTURES}
    assert names == expected
    for name, mode, realism, level, shadows, integration, motion_blend, path in paths:
        suffix = ("-flatmesh" if level == 0
                  else "-hardshadow" if shadows == "hard"
                  else "-nocomposite" if integration == 0
                  else "-strong" if integration == 3
                  else "-tickmotion" if motion_blend else "")
