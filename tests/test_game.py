...
def test_init_golden(data_dir, profile):
...
def test_stage_actors_spawned(data_dir, profile):
    game = init_game(data_dir, profile, hero=0)
...
def test_tick(data_dir, profile):
    game = init_game(data_dir, profile, hero=0)
    game.timer = 0
    game_step_tick(game)
    assert game.timer == 1
...
def test_phase1_delete_gates(data_dir, profile, life, life_mode, stage, room, kept):
    game = init_game(data_dir, profile, hero=0)
...
def test_phase2_life_mode_minus1_not_spawned(data_dir, profile):
    game = init_game(data_dir, profile, hero=0)
    world_idx = _stage_free_world_idx(game)
...
def test_spawn_does_not_touch_found_flag(data_dir, profile):
    game = init_game(data_dir, profile, hero=0)
...
def test_both_heroes_share_fitd_initial_state_except_cvar_and_archives(data_dir, profile, hero):
...
def test_load_floor_threads_the_profile_and_does_not_cache(data_dir, profile):
