...
from PyAitD.engine.script.effects import AddMessage, BeginTake, LifeFrame
from PyAitD.engine.script.game import init_game
from PyAitD.engine.script.interaction import advance_messages, drain_immediate_effects
import pytest
...
def test_begin_take_runs_after_parent_frame_is_stacked(data_dir, profile, monkeypatch):
    ...
    seen = []
    monkeypatch.setattr("PyAitD.engine.script.interaction.inventory.begin_take", lambda g, i: seen.append((i, len(g.life_stack))) or False)
