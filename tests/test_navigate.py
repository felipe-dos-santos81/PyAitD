...
def test_the_run_commit_window_aims_the_hero_without_moving_him():
    # Run is decided at press time and a press cannot see the second press
    # coming, so an intent that advances immediately can only ever walk first.
    # The window is what lets a double press land as a run from the first step.
    intent = NavIntent(dest_x=9000, dest_z=0, room=0, waypoints=[(9000, 0)],
                       hold_until=10)
    intent.path_room = 0
    game = _Game(intent)
    decision = decide(game, _actor(0, 0, beta=0), None)
    assert decision.advance is False
    assert decision.arrived is False and decision.abandoned is False
    assert decision.joyd == 0


def test_the_hero_advances_once_the_run_commit_window_has_passed():
    intent = NavIntent(dest_x=9000, dest_z=0, room=0, waypoints=[(9000, 0)],
                       hold_until=10)
    intent.path_room = 0
    game = _Game(intent)
    game.timer = 10
    assert decide(game, _actor(0, 0, beta=0), None).advance is True


def test_waiting_out_the_run_commit_window_is_not_failing_to_make_progress():
    # The window is placed before the give-up test for this reason: standing
    # still is what it asks for, so counting those ticks as a stall would
    # abandon a click that had not been given a chance to move yet.
    intent = NavIntent(dest_x=9000, dest_z=0, room=0, waypoints=[(9000, 0)],
                       hold_until=STALL_TICKS * 3)
    intent.path_room = 0
    game = _Game(intent)
    actor = _actor(0, 0, beta=0)
    for game.timer in range(STALL_TICKS * 2):
        assert decide(game, actor, None).abandoned is False
    assert intent.stall_ticks == 0


def test_a_click_the_hero_is_already_standing_on_dispatches_inside_the_window():
    # The window delays walking, not acting: it sits after the arrival test so
    # a door or a pickup under the hero's feet still fires on the press.
    intent = NavIntent(dest_x=0, dest_z=0, room=0, waypoints=[(0, 0)],
                       hold_until=10)
    intent.path_room = 0
    assert decide(_Game(intent), _actor(0, 0, beta=0), None).arrived is True
