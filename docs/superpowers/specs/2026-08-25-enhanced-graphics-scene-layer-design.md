...
| `asset_resolver.py` | `AssetResolver(assets, override_dir=None)`: `background(floor, cam)`, `body(num)`, `palette()`. Override lookup by convention path, fallback to `assets.py` / `floor.py`. One isolated PNG-loading function touches pygame's image module. | PNG load only |
