...
proof-graphics: install ## Graphics proof: attic + combat fixtures at scale 4 per shading mode x realism preset, plus a flat-mesh pair, a hard-shadow pair, an un-composited pair, an over-composited pair, a motion-blended tickmotion pair, a painted pair, an SSAO-off nossao pair, a room-shadow roomshadow pair and an un-hazed nohaze pair, to docs/graphics-proof/ (motion=tick renders the tickmotion pair unblended; occlusion= and atmosphere= set the main renders' modes, the nossao and nohaze pairs are always off)
	$(PYTHON) tools/prove_graphics.py "$(data)" $(if $(motion),--motion "$(motion)") $(if $(occlusion),--occlusion "$(occlusion)") $(if $(atmosphere),--atmosphere "$(atmosphere)")
...
export-textures: install ## Export every camera background + 5 KILLED_SORCERER alts + palette + ITD_RESS screens + guides + layout sidecars + manifest schema 4, then survey palette ramps + body usage into <out>/materials-survey and emit PyAitD/render/materials.json (out=data/aitd1/textures, floors=0-7, scale=4, force=1, screens=0 skips screens, uvs=0 skips the actor UV bake, materials=0 skips the materials stages, vision=1 asks Gemini through agy about uncertain ramps, model=, threshold=0.8)
